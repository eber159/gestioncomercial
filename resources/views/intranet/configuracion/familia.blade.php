@section('title-page')
	ErpWeb | Gestión Familias
@stop

@extends('intranet.plantilla_int')

@section('titulo-form')
	Gestión de Familias <small>Panel de Control</small>
@stop
						...
						<div class="box-body">
							...
							<div class="col-md-12" style="display: none"> 
								<div class="form-group">
									...
									<select class="form-control" id="cmbtipomaterial" name="cmbtipomaterial" placeholder="Seleccione" style="width: 100%">
							...
							<div class="col-md-12">
								<div class="form-group">
									...
									<input type="text" class="form-control" id="nombre" name="nombre" placeholder="Ingrese Familia" style="width: 100%">
								</div>
							</div>
						</div>
					  <!-- /.box-body -->
	...
	<script type="text/javascript">
		$(function () {
			...
			$("#btnGuardar").on("click", function(event) {
				...
				var resp = Guardar(form,'familia/Guardar');
				if(resp=="ok"){
		...
		function CargarComboTipoMaterial() {
            var $combo = $("#cmbtipomaterial");
            $combo.empty();
            $.post('tipomaterial/Listar',
                    function (data) {
					...
					if (data !== null && typeof data === 'object') {
						$.each(data.lista, function (key, val) {
							var obj = {"Id": val["id"]
									,"Sel": ""
									,"Codigo": val["codigo"]
									,"TipoMaterial": val["nombretipomaterial"]
									,"Nombre": val["nombre"]
									,"Acciones":"<center><div class='hidden-sm hidden-xs action-buttons'>"
													+"<a href='javascript:;' onclick='Editar(\"familia/Leer\","+val["id"]+")' class='blue'>"
														+"<i class='ace-icon fa fa-pencil bigger-130'></i>"
													+"</a>&nbsp;"
													+"<a href='javascript:;' onclick='Eliminar(\"familia/Eliminar\","+val["id"]+")' class='red'>"
		...
		function InicializarTabla() {
			...
			var Table = $('#tblDatos').dataTable({
				...
				"aoColumns": [{
						...
						"sWidth": "20%", "mDataProp": "TipoMaterial"
					},{
		...
		function CargarDatos(val){
			...
			$("#cmbtipomaterial").val(val["idtipomaterial"]);
