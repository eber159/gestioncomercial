@section('title-page')
	ErpWeb | Gestión Botones
@stop

@extends('intranet.plantilla_int')

@section('titulo-form')
	Gestión de Botones <small>Panel de Control</small>
@stop

@section('subtitulo-form')
	<ol class="breadcrumb">
		<li><a href="#"><i class="fa fa-dashboard"></i> Botones</a></li>
			...
			<div class="col-md-12">
				<div class="box box-primary">
					<form role="form" id="frmRegistro" enctype="multipart/form-data">
						...
						<input type="hidden" class="form-control" id="idtipo" name="idtipo" placeholder="Ingrese tipo" value="2" style="width: 100%">
						<div class="box-body">

							<div class="col-md-6">
								<div class="form-group">
									<label for="">URL (LINK) </label>
									<input type="text" class="form-control" id="url" name="url" placeholder="Ingrese Título" style="width: 100%" >
								</div>
							</div>
							<div class="col-md-12">
								<div class="form-group">
									<label for="">Descripción</label>
									<input type="text" class="form-control" id="descripcion" name="descripcion" placeholder="Ingrese Descripcion" style="width: 100%">
								</div>
							</div>
	...
	<script type="text/javascript">
		$(function () {
			...
			$("#btnGuardar").on("click", function(event) {
				...
				var resp = Guardar(form,'enlaces/Guardar');
				if(resp=="ok"){
				...
				success: function (data) {
					...
					if (data !== null && typeof data === 'object') {
						$.each(data.lista, function (key, val) {
							var obj = {"Id": val["id"]
									,"URL": "<a href='#' class='btn btn-default btn-xs' onclick='VerImagen("+val["id"]+")' ><i class='fa fa-image'></i></a>"+val["url"]
									,"Descripcion": val["descripcion"]
									,"Acciones":"<center>"
													+"<a href='javascript:;' class='btn btn-primary btn-xs' onclick='Editar(\"enlaces/Leer\","+val["id"]+")' class='blue'>"
														+"<i class='ace-icon fa fa-pencil bigger-130'></i>"
													+"</a>&nbsp;"
													+"<a href='javascript:;' class='btn btn-danger btn-xs' onclick='Eliminar(\"enlaces/Eliminar\","+val["id"]+")' class='red'>"
		...
		function CargarDatos(val){
			...
			var RutaImagen2 = "{{config('constants.rutapublica.url')}}"+val['enlaces'];
        ...
        $("#frmRegistro").submit(function(e) {
            var url = "enlaces/Guardar"; // the script where you handle the form input.  
