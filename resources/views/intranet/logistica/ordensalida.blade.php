@section('title-page')
	ErpWeb | Gestión Orden Salida
@stop

@extends('intranet.plantilla_int')

@section('titulo-form')
	Gestión de Orden Salida <small>Panel de Control</small>
@stop

@section('subtitulo-form')
	<ol class="breadcrumb">
		<li><a href="#"><i class="fa fa-dashboard"></i>Orden Salida</a></li>
...
@section('stripts_especific')
	...
	<script>
		...
		var tipoingreso = {{config('constants.tipoordeningresosalida.salida')}};
	...
	<script type="text/javascript">
		...
		function CargarComboMovimientoInventario() 
		{
		  ...
		  $combo.empty();
		  $.post('../configuracion/categoria/Listar',{"grupo":"MOVIMIENTO_INVENTARIO", "abreviatura":"S"},
		          function (data) 
		          {
		            $.each(data.lista, function (index, item) {
		            $combo.append("<option value='" + item.id + "'>"
