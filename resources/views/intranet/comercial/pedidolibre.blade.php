...
@extends('intranet.plantilla_externa')
...
@section('stripts_especific')
	...
	<script type="text/javascript">
		...
		function GuardarVenta(){
			if(validarGuardarPedido()){
				...
				$.ajax({
					...
					async: false,
					data: obj,
					url: "{{ URL::to('intranet/comercial/ordenpedido/GuardarExterno') }}",
					dataType: "json",
					...
					success: function (data) {
						if(data.success == true){
							bootbox.alert("GRACIAS POR REGISTRAR SU PEDIDO, CÓDIGO GENERADO: " + data.resultadocodigo);
							$('#frmRegistro')[0].reset();
		...
		function Listar() {
			...
			var url = "{{ URL::to('intranet/comercial/ordenpedido/Listar') }}";
			var info = "";
			$.ajax({
				type: "POST",
				async: false,
				data: { "idtrabajador": "{{ Session::get('idtrabajador') }}", "fechaini":$("#fechaini").val(),"fechafin":$("#fechafin").val() },
				url: url,
				dataType: "json",
				beforeSend: function (data) {
					$("#divCargando").show();
	                    ...
	                    "columns": [
	                                ...
	                                { "data": null, "width": "8%",
	                                	"render": function (data, type, row) {
											...
											return boton_aprobar+boton_despachar+boton_rechazar
			...
			bootbox.confirm("¿Seguro que deseas Rechazar el pedido?", function(result) {
				if(result){
					$.ajax({
						...
						url: "{{ URL::to('intranet/comercial/ordenpedido/RechazarPedido') }}" ,
						dataType: "json",
						beforeSend: function (data) {
        ...
        function CargarComboEstado() {
            var $combo = $("#idestado");
            $combo.empty();
            $.post("{{ URL::to('intranet/configuracion/categoria/Listar') }}",{"grupo":"ESTADO_PEDIDO"},
                    function (data) {
                        $.each(data.lista, function (index, item) {
        ...
        function CargarComboProducto() {
            var $combo = $("#idproductobusqueda");
            $combo.empty();
            $.post("{{ URL::to('intranet/configuracion/material/Listar') }}",
                    function (data) {
                        $combo.append("<option value=''>Seleccione</option>");
                        $.each(data.lista, function (index, item) {
                            $combo.append("<option value='" + item.id + "'>"
        ...
        function BuscarxCodigo() {
			...
			$.ajax({
				...
				url: "{{ URL::to('intranet/configuracion/material/ListarFiltros') }}",
				dataType: "json",
				beforeSend: function (data) {
		...
		function CargarComboProductoNombre(val) {
            var $combo = $("#idproductobusqueda");
            $combo.empty();
            $.post("{{ URL::to('intranet/configuracion/material/ListarFiltros') }}",{"tipo":"nombre","filtro":val},
                    function (data) {
                        $combo.append("<option value=''>Seleccione</option>");
                        $.each(data.lista, function (index, item) {
                            $combo.append("<option value='" + item.id + "'>"
		...
		function CargarComboUnidadMedida(val) {
            ...
            $.post("{{ URL::to('intranet/configuracion/unidadmedida/Listar') }}",
                    function (data) {
                        $.each(data.lista, function (index, item) {
                            $combo.append("<option value='" + item.id + "'>"
                                    + item.nombre + "</option>");
		...
		function consultarPrecios(idproducto) {
			...
			url: "{{ URL::to('intranet/configuracion/precioproducto/Listar') }}",
			dataType: "json",
			beforeSend: function (data) {
				$("#divError").hide();
			},
		...
		{
		  var url = "{{ URL::to('intranet/logistica/inventario/Listar') }}"
		  $.ajax({
		    type: 'POST',
		    async: false,
		    data: { 'idproducto': idproducto },
		...
		function CargarTipoDocumento() {
            ...
            $combo.empty();
            $.post("{{ URL::to('intranet/configuracion/categoria/Listar') }}",{"grupo":"TIPO_DOCUMENTO"},
			function (data) {
				$.each(data.lista, function (index, item) {
		...
		function CargarSeries() {
            var $combo = $("#serie");
            $combo.empty();
            $.post("{{ URL::to('intranet/configuracion/serie/Listar') }}",{"idtipodocumento": $("#cmbtipodocumentomodal").val() ,"indcompraventa":"V"},
			function (data) {
				$.each(data.lista, function (index, item) {
        ...
        function CargarUltimoCorrelativo() {
            $.post("{{ URL::to('intranet/configuracion/serie/ListarCorrelativo') }}",{"idtipodocumento": $("#cmbtipodocumentomodal").val() ,"indcompraventa":"V","serie":$("#serie").val()},
			function (data) {
				$.each(data.lista, function (index, item) {
        ...
        function CargarComboAlmacenSalidas() {
		  ...
		  $combo.empty()
		  $.post("{{ URL::to('intranet/logistica/almacen/Listar') }}",
		    function (data) {
		      $.each(data.lista, function (index, item) {
		        $combo.append("<option value='" + item.id + "'>"
		          + item.nombre + '</option>')
		...
		function CargarPedidos(id) {
			...
			$.ajax({
				...
				async: false,
				data: {"id":id},
				url: "{{ URL::to('intranet/comercial/ordenpedido/Leer') }}",
				dataType: "json",
				beforeSend: function (data) {
		...
		function GenerarOrdenSalida(){
			...
			$.ajax({
				...
				async: false,
				data: obj,
				url: "{{ URL::to('intranet/comercial/ordenventa/GenerarOrdenSalida') }}",
				dataType: "json",
				beforeSend: function (data) {
