@section('title-page')
	ErpWeb | Gestión Boletas (Ventas)
@stop

@extends('plantilla_int')

@section('titulo-form')
	Gestión de Boletas (Ventas) <small>Panel de Control</small>
@stop

@section('subtitulo-form')
	<ol class="breadcrumb">
		<li><a href="#"><i class="fa fa-dashboard"></i> Boletas (Ventas)</a></li>
...
@section('stripts_especific')
	...
	<script type="text/javascript">
        ...
        function GuardarBoletaVenta(){
			...
			var obj = {
				"id": $("#id").val()
				,"idtipodocumento": {{ config('constants.tipodocumento.boleta') }}
		...
		function Listar() {
			...
			$.ajax({
				type: "POST",
				async: false,
				data: {"fechaini":$("#fechaini").val(),"fechafin":$("#fechafin").val(),"idtipodocumento":{{config('constants.tipodocumento.boleta')}},"idtipocompraventa": 1},
				url: url,
				dataType: "json",
				beforeSend: function (data) {
